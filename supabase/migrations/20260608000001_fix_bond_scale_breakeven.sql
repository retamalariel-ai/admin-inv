-- =============================================================================
-- MIGRACIÓN: Fix escala de precios de bonos + columnas break-even
--
-- Cambios respecto a 006_views.sql:
--   1. price_quotes.price para bonos ahora es precio por VN 1 (normalizado en
--      el ingreso desde PPI). Se elimina la división /100 en las fórmulas.
--   2. Nuevas columnas: break_even_price_ars, break_even_price_usd,
--      spread_vs_breakeven_ars, spread_vs_breakeven_pct.
-- =============================================================================
...
base AS (
  SELECT
    ...
    -- market_value_ars: Bonos: price es ARS por VN 1; Acciones: ARS/unidad; Cripto: USD/unidad × FX
    CASE
        ...
        CASE
          WHEN a.currency = 'ARS' THEN
            pos.quantity_held
              ...
              * a.current_residual_factor
          WHEN a.currency IN ('USD_MEP','USD_CCL','USD_CABLE') THEN
            pos.quantity_held
              * COALESCE(lp.current_price, 0)
              * a.current_residual_factor
