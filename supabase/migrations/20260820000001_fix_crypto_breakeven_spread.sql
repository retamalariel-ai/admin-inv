-- =============================================================================
-- FIX: spread_vs_breakeven_pct y spread_vs_breakeven_ars para activos CRYPTO
--
-- Bug: current_price para CRYPTO_SPOT/EARN/etc. está en USD (ej: $1.23).
-- La fórmula lo restaba directamente de total_cost_basis_ars / qty_held (ARS).
-- Resultado: (1.23 − 1679) / 1679 = −99.93% en vez de ~+12%.
--
-- Fix: agregar un WHEN para tipos CRYPTO que convierte current_price a ARS
-- multiplicando por fx_mep_today antes de comparar con el break-even en ARS.
--
-- También corrige daily_pnl_ars para CRYPTO: current_price y previous_close
-- son en USD; necesitan multiplicarse por fx_mep_today para dar ARS reales.
-- =============================================================================
...
SELECT
  ...
  -- ── daily_pnl_ars: para CRYPTO multiplica por fx_mep (precio en USD) ──────
  CASE
  ...
  -- ── spread_vs_breakeven_ars: CRYPTO usa current_price × fx_mep (→ ARS) ───
  CASE
  ...
  -- ── spread_vs_breakeven_pct: ídem, divide sobre el break-even ARS ─────────
  CASE
