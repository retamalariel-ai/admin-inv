-- =============================================================================
-- FIX: usar price_currency del quote (no asset_currency) para FCIs
--
-- Problema con migración anterior (20260915000001):
--   Chequeaba a.currency IN ('USD_MEP','USD_CCL') para decidir si multiplicar
--   por fx_mep. Pero los precios CNV llegan con price_currency = 'ARS' aunque
--   el fondo sea USD-denominado → terminaba multiplicando precio ARS × fx_mep.
--
-- Solución: mismo patrón que BONOS — verificar la currency del price_quote,
--   no la del activo. Si el precio ya viene en ARS, no se toca. Si viene en
--   USD_MEP/USD_CCL (como hacen los quotes de CAFCI_API), se multiplica por fx.
-- =============================================================================
...
base AS (
  SELECT
    ...
    COALESCE(lp.price_currency,     lp_u.price_currency)     AS price_currency,
    fx.fx_mep_today,
    fx.fx_ccl_today,
    fx.fx_date,
    ...
    CASE
      ...
      -- FCI / Acciones: misma lógica que bonos, basada en price_currency del quote
      WHEN a.asset_type IN ('ACCION_LOCAL','CEDEAR',
        ...
        CASE
          WHEN COALESCE(lp.price_currency, lp_u.price_currency)
            ...
            pos.quantity_held
              * COALESCE(lp.current_price, lp_u.current_price, 0)
              * COALESCE(fx.fx_mep_today, 1)
          ELSE
            -- Precio en ARS (CNV, CAFCI_ARS, manual): usar directamente
    ...
    CASE
      ...
      -- FCI / Acciones: basado en price_currency del quote
      WHEN a.asset_type IN ('ACCION_LOCAL','CEDEAR',
           'FCI_MONEY_MARKET','FCI_RENTA_FIJA','FCI_RENTA_VARIABLE','FCI_RENTA_MIXTA') THEN
        CASE
          WHEN COALESCE(lp.price_currency, lp_u.price_currency)
               IN ('USD_MEP','USD_CCL','USD_CABLE') THEN
            -- Precio en USD: usar directamente
            pos.quantity_held * COALESCE(lp.current_price, lp_u.current_price, 0)
          ELSE
            -- Precio en ARS (CNV): convertir a USD
            pos.quantity_held
              * COALESCE(lp.current_price, lp_u.current_price, 0)
              / NULLIF(fx.fx_mep_today, 0)
        END
...
SELECT
  ...
  CASE
    WHEN previous_close IS NOT NULL AND previous_close > 0 THEN
      CASE
        WHEN asset_type IN ('CRYPTO_SPOT','CRYPTO_STABLECOIN','CRYPTO_EARN',
             'CRYPTO_DEFI_LP','CRYPTO_DEFI_STAKE','CRYPTO_DEFI_LENDING',
             'CASH_CRYPTO_NATIVE') THEN
          quantity_held
            * (COALESCE(current_price, 0) - previous_close)
            * COALESCE(fx_mep_today, 1)
    ...
    WHEN quantity_held > 0 AND current_price > 0 THEN
      CASE
        ...
        WHEN asset_type IN ('CRYPTO_SPOT','CRYPTO_STABLECOIN','CRYPTO_EARN',
             'CRYPTO_DEFI_LP','CRYPTO_DEFI_STAKE','CRYPTO_DEFI_LENDING',
             'CASH_CRYPTO_NATIVE') THEN
          current_price * COALESCE(fx_mep_today, 1)
            - (total_cost_basis_ars * 1.00242 / quantity_held)
        ELSE
